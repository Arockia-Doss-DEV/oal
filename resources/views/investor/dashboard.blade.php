                                            ...
                                            <tbody>
                                                @if ($payouts->count() > 0)
                                                    ...
                                                    @foreach ($payouts as $key => $payout)
                                                        @foreach ($payout['PayoutAs'] as $key2 => $payoutData)

                                                        <tr>
                                                            <td>{{ $key2+1 }}</td>
                                                            <td>{{ date('d-M-y', strtotime($payoutData->created_at)) }}</td>
                                                            <td>$ {{ $payoutData->redemption_amount }}</td>
                                                            <td>$ {{ $payoutData->cumulative_amount }}</td>
                                                            <td><a href="{{ asset('storage/'.$payoutData->redemption_file) }}"  download class="btn btn-base btn-rounded btn-fw mt-1 mr-1"> Download </a></td>
                                                        </tr>
